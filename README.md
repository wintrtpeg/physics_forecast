...
|---|---|---|---|
| 물리모델 | 2.51 mg/Sm³ | **2.58 mg/Sm³** | −0.62 |
...
| 항목 | 개발용 데이터 | **처음 보는 데이터** |
|---|---|---|
| 고치기 전 | 첫 바이트에서 `UnicodeDecodeError`. UTF-8 로 바꿔도 26개 컬럼 중 25개와 석 달 치 행이 **조용히** 사라짐 | — |
| 파일 결함 (중복 행 = 주입) | 72 = 72, 해석 못 한 시각 0 | 72 = 72, 0 |
| 센서 이상점 | 1,777점 **전부 탐지, 오탐 0** | 1,782점 **전부, 오탐 0** (교정 시각이 바뀌어도) |
| 계측기 진단 (보존식) | 헤더 유량계 +3.0% (주입 3%) · 예비 온도계 +0.451 ℃ (주입 0.45) | +2.3% (주입 2.34%) · +0.467 ℃ (주입 0.465) |
| 증설 후 예측, 참값 대비 RMSE | 물리 **2.04** · ML 다항 11.83 · ML 부스팅 19.66 | 물리 **3.50** · ML 다항 9.05 · ML 부스팅 21.75 |
| 관리기준 초과 시간 적중 / 오경보 | 물리 311/328, 132 · 다항 327/328, **855** · 부스팅 **0/328** | 물리 290/394, 90 · 다항 343/394, **827** · 부스팅 **0/394** |
| 숨긴 사건 4건 (잔차 변화점) | 4건 | **2건** (사흘 간격 반대 방향 계단은 상쇄되어 못 봄) |

물리모델 오차는 두 데이터 모두 분석계 자체 오차(2.83, 3.93)보다 작습니다. 트리 모델은
학습 최댓값에서 평평해져 초과를 **한 시간도** 못 잡고, 다항식은 치솟아 오경보를 쏟아냅니다.
물리모델도 처음 보는 데이터에서는 나빠졌고, 변화점 탐지는 개발용 성적이 그 데이터에 맞춘
결과였음이 드러났습니다. 보정된 파라미터 개별 값은 모델 오차를 흡수한 **유효값**이라
설비 진단에 그대로 쓰면 안 됩니다. 이 한계들도 문서에 그대로 적었습니다.
