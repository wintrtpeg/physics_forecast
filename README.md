...
|---|---|---|---|---|
| 물리모델 | 2.47 mg/Sm³ | **2.52 mg/Sm³** | −0.33 | 0.919 |
| ML 기준모델(2차 다항) | 2.45 mg/Sm³ | **2.70 mg/Sm³** | −0.63 | 0.907 |

물리모델은 외삽 구간에서 오차가 **거의 늘지 않습니다**. 학습 구간을 벗어난다는
개념 자체가 없기 때문입니다. 이 깨끗한 예제는 검증 구간이 학습 구간과 가까워 ML 도
10% 나빠지는 데 그칩니다. 증설이 끼는 현실적인 경우는 아래
[현장형 더미 데이터 테스트](#현장형-더미-데이터-테스트--지저분한-데이터에서도-되는가)를 보세요.
...
헤더 유량계 +3% 게인, 비계측 레시피 변경, 충전재 오염과 세정, 그리고 **6월 장비 증설**.

| 항목 | 결과 |
|---|---|
| 고치기 전 | 첫 바이트에서 `UnicodeDecodeError`. UTF-8 로 바꿔도 26개 컬럼 중 25개와 석 달 치 행이 **조용히** 사라짐 |
| 파일 결함 | 전부 찾아 고치고 보고 (중복 72행 = 주입 72행, 해석 못 한 시각 0) |
| 센서 이상 1,777점 | **전부 탐지, 오탐 0** (정상 점 161만 개 중). 펌프 정지(값 0)는 지우지 않음 |
| 계측기 진단 | 보존식으로 "헤더 유량계 **+3.0%**" (주입 3%), "예비 온도계 **+0.451 ℃**" (주입 0.45) |
| 증설 후 여름 예측 (참값 대비 RMSE) | 물리 **2.04** · ML 다항 11.83 · ML 부스팅 19.66 mg/Sm³ |
| 관리기준 초과 328시간 | 물리 311시간 적중 · 부스팅 **0시간** |
| 숨긴 사건 4건 (레시피·전송기·세정·분석계 교체) | 잔차 변화점으로 **4건 모두**, 유형까지 맞게 |

물리모델 오차는 분석계 자체 오차(2.83)보다 작습니다. 다만 보정된 파라미터 개별 값은
모델 오차를 흡수한 **유효값**이라 진짜 값과 다릅니다 — 예측에는 써도 설비 진단에
그대로 쓰면 안 됩니다. 이 한계도 문서에 그대로 적었습니다.
